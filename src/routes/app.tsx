...
async function fetchTranscript(videoId: string): Promise<TranscriptSegment[]> {
  // Strategy 1: Try youtranscript.com API
  try {
  ...
  // Strategy 2: Extract from YouTube page directly
  return await extractTranscriptFromYouTube(videoId);
}
...
async function extractTranscriptFromYouTube(
  ...
  const captionTracks = captions.captionTracks as
...
function parseTimedText(xml: string): TranscriptSegment[] {
  ...
  // Try alternate format: <p t="..." d="...">text</p>
  if (segments.length === 0) {
    const altRegex = /<p\s+t="([\d.]+)"\s+d="([\d.]+)"[^>]*>(.*?)<\/p>/g;
  ...
  if (segments.length === 0) {
    throw new Error("No transcript available — could not parse captions.");
  }
